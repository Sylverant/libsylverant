...
int sylverant_char_fetch(sylverant_dbconn_t *conn, uint32_t gc, int slot,
    ...
    /* Build the query. */
    r = snprintf(query, 256, "SELECT data FROM character_data WHERE "
    ...
    if(!sylverant_db_query(conn, query)) {
        ...
        if(row_ct && rv) {
            ...
            memcpy(rv, row[0], sizeof(sylverant_character_t));
        }
